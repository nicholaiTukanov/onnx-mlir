...
/// Singleton class to construct an SMaLL accelerator.
class SMaLLAccelerator final : public Accelerator {
private:
  static SMaLLAccelerator *instance;
  SMaLLAccelerator();

public:
  /// Singleton should not be clonable or assignable.
  SMaLLAccelerator(SMaLLAccelerator &) = delete;
  void operator=(const SMaLLAccelerator &) = delete;

  ~SMaLLAccelerator();

  /// Creates an instance on the first invocation. Subsequent invocations
  /// return the existing instance.
  static SMaLLAccelerator *getInstance();

  /// Define classof to be able to use isa<>, cast<>, dyn_cast<>, etc.
  static bool classof(const Accelerator *accel) {
    return accel->getKind() == Accelerator::Kind::SMaLL;
  }
  static bool classof(const SMaLLAccelerator *) { return true; }
